...
# AI Backend
if start_service "AI Backend" "node src/backend/simple-test-server.js" 3006 "logs/ai-backend.log"; then
    ((services_started++))
fi

# Blockchain Service
if start_service "Blockchain Service" "node src/backend/blockchain-service.js" 3007 "logs/blockchain.log"; then
    ((services_started++))
fi

# Admin Backend
if start_service "Admin Backend" "node src/backend/admin-backend.js" 3008 "logs/admin-backend.log"; then
    ((services_started++))
fi

# Frontend Server
if start_service "Frontend Server" "node src/frontend/simple-frontend-server.js" 8081 "logs/frontend.log"; then
    ((services_started++))
fi

# SD AI Mock Server
if start_service "SD AI Mock" "python3 src/backend/simple-sd-ai-mock.py" 8000 "logs/sd-ai.log"; then
    ((services_started++))
fi
